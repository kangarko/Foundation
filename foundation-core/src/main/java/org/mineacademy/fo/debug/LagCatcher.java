...
public final class LagCatcher {
	...
	public static void performanceTest(int cycles, String name, Runnable code) {
		...
		System.out.println("Test '" + name + "' took " + MathUtilCore.formatTwoDigits(finishAndCalculate(name + "-whole")) + " ms. Average " + MathUtilCore.average(lagMap) + " ms");
		...
		if (!durationsMap.isEmpty()) {
			for (final Map.Entry<String, List<Long>> entry : durationsMap.entrySet()) {
				...
				System.out.println("\tSection '" + section + "' took " + MathUtilCore.formatTwoDigits(duration / 1_000_000D));
			}
