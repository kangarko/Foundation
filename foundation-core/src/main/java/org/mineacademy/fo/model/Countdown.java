...
public abstract class Countdown implements Runnable {
		...
		if (this.secondsSinceStart < this.countdownSeconds)
			try {
				...
				try {
					...
					CommonCore.log("Unable to handle onTickError, got " + t + ": " + tt.getMessage());
				}
	...
	public final void launch() {
		ValidCore.checkBoolean(!this.isRunning(), "Task " + this + " already scheduled!");
		ValidCore.checkBoolean(!this.paused, "You cannot launch a countdown that is paused!");

		this.task = Platform.runTaskTimer(START_DELAY, TICK_PERIOD, this);
	...
	public final void pause() {
		ValidCore.checkBoolean(this.isRunning(), "Countdown must be scheduled in order to pause it!");
	...
	public final void resume() {
		ValidCore.checkBoolean(this.paused, "Countdown must be paused in order to resume it!");
