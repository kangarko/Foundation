...
public final class Replacer {
	...
	private static String replaceVariables(String message, SerializedMap variables, Matcher matcher) {
		while (matcher.find()) {
			...
			if (value != null) {
				final boolean emptyColorless = CommonCore.removeColors(value).isEmpty();
				value = value.isEmpty() ? "" : (frontSpace && !emptyColorless ? " " : "") + CommonCore.colorize(value) + (backSpace && !emptyColorless ? " " : "");
