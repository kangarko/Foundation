...
public final class RegionTool extends VisualTool {
	...
	@Getter
	@Setter
	private static String blockName = "&f[&a{type} Region Point&f]";
	...
	protected String getBlockName(final Block block, final Player player) {
		...
		return blockName.replace("{type}", region.isPrimary(loc) ? "Primary" : region.isSecondary(loc) ? "Secondary" : "Unknown");
	}
