...
public abstract class OfflineRegionScanner {
	...
	private void scanFile(File file, Queue<File> queue) {
		...
		if (this.fastMode)
			...
			Platform.runTask(WAIT_TIME_BETWEEN_SCAN_SECONDS, () -> this.schedule0(queue));
	}
...
class RegionAccessor {
	...
	static {
		...
		try {
			final Class<?> regionFileClass = Remain.getNMSClass("RegionFile", "net.minecraft.world.level.chunk.storage.RegionFile");
			...
			isChunkSaved = atleast1_14 ? regionFileClass.getMethod("b", Remain.getNMSClass("ChunkCoordIntPair", "net.minecraft.world.level.ChunkCoordIntPair"))
