		...
		try {
			...
			if (!ReflectionUtil.isClassAvailable("net.kyori.adventure.text.minimessage.MiniMessage"))
				...
				if (ReflectionUtil.isClassAvailable("net.kyori.adventure.audience.Audience")) {
					...
					this.loadLibrary("net.kyori", "adventure-text-minimessage", "5.0.0");
	...
	public final void onEnable() {
		...
		try {
				...
				public boolean isIgnored(final Class<?> clazz, final boolean printWarnings) {
					...
					if (PacketListener.class.isAssignableFrom(clazz) && !HookManager.isPacketEventsLoaded()) {
						if (printWarnings && !clazz.equals(BukkitPacketListener.class)) {
							CommonCore.warning("**** WARNING ****");
							CommonCore.warning("The following class requires the PacketEvents plugin and won't be registered: " + clazz.getSimpleName()
									+ ". To hide this message, put @AutoRegister(hideIncompatibilityWarnings=true) over the class.");
						}
			...
			if (SimpleSettings.REGISTER_REGIONS) {
				ValidCore.checkBoolean(DiskRegion.hasCreatedPlayerRegionGetter(), "Alert author of " + this.getName() + " to call DiskRegion#setCreatedPlayerRegionGetter in onPluginLoad()");
				ValidCore.checkBoolean(DiskRegion.hasCreatedPlayerRegionResetter(), "Alert author of " + this.getName() + " to call DiskRegion#sasCreatedPlayerRegionResetter in onPluginLoad()");
	...
	/*private void scanEnumsForErrors() {
		for (final CompAttribute comp : CompAttribute.values())
			try {
				...
				if (comp.getNmsName() != null)
					CommonCore.log("Invalid CompAttribute " + comp.name());
			}
		...
		for (final CompColor comp : CompColor.values())
			try {
				...
				CommonCore.log("Invalid CompColor " + comp.getName());
			}
		...
		for (final CompItemFlag comp : CompItemFlag.values())
			try {
				...
				CommonCore.log("Invalid CompItemFlag " + comp);
			}
		...
		for (final CompParticle comp : CompParticle.values())
			try {
				...
				if (!comp.isRemoved())
					CommonCore.log("Invalid CompParticle " + comp);
			}
		...
		if (MinecraftVersion.atLeast(V.v1_21))
			for (final CompSound comp : CompSound.values())
				try {
					...
					CommonCore.log("Invalid CompSound " + comp.name());
				}
		...
		for (final CompVillagerProfession comp : CompVillagerProfession.values())
			try {
				...
				CommonCore.log("Invalid CompVillagerProfession " + comp);
			}
		...
		for (final CompVillagerType comp : CompVillagerType.values())
			try {
				...
				CommonCore.log("Invalid CompVillagerType " + comp);
			}
	}
	...
	private void scanModernEnumsForUpdates() {
		for (final Attribute bukkit : Attribute.values())
			try {
				...
				CommonCore.log("Missing CompAttribute for Bukkit's " + bukkit.name());
			}
		...
		for (final Biome bukkit : Biome.values())
			try {
				...
				CommonCore.log("Missing CompBiome for Bukkit's " + bukkit.name());
			}
	
		for (final EntityType bukkit : EntityType.values())
			try {
				if (CompEntityType.fromName(bukkit.name()) == null)
					throw new IllegalArgumentException();
	
			} catch (final IllegalArgumentException ex) {
				CommonCore.log("Missing CompEntityType for Bukkit's " + bukkit.name());
			}
		...
		for (final DyeColor bukkit : DyeColor.values())
			try {
				...
				CommonCore.log("Missing CompColor for Bukkit's " + bukkit.name());
			}
		...
		for (final Enchantment bukkit : Enchantment.values())
			try {
				...
				CommonCore.log("Missing CompEnchantment for Bukkit's " + bukkit);
			}
		...
		for (final ItemFlag bukkit : ItemFlag.values())
			try {
				...
				CommonCore.log("Missing CompItemFlag for Bukkit's " + bukkit);
			}
		...
		for (final Material bukkit : Material.values())
			try {
				...
				CommonCore.log("Missing CompMaterial for Bukkit's " + bukkit);
			}
		...
		for (final Particle bukkit : Particle.values())
			try {
				...
				CommonCore.log("Missing CompParticle for Bukkit's " + bukkit);
			}
		...
		for (final PotionEffectType bukkit : PotionEffectType.values())
			try {
				...
				CommonCore.log("Missing CompPotionEffectType for Bukkit's " + bukkit);
			}
		...
		for (final Sound bukkit : Sound.values())
			try {
				...
				CommonCore.log("Missing CompSound for Bukkit's " + bukkit.name());
			}
		...
		for (final Villager.Profession bukkit : Villager.Profession.values())
			try {
				...
				CommonCore.log("Missing CompVillagerProfession for Bukkit's " + bukkit);
			}
		...
		for (final Villager.Type bukkit : Villager.Type.values())
			try {
				...
				CommonCore.log("Missing CompVillagerType for Bukkit's " + bukkit);
			}
	}*/
