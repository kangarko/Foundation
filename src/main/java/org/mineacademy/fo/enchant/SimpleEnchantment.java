 ...
 * <p>
 * DISCLAIMER: Minecraft is not built for your custom enchants. The enchant name
 * is stored
 * on the client so it wont show anything for custom enchantments - Foundation
 * will do its
 * best to intercept the set slot packet and inject the lore manually (requires
 * ProtocolLib),
 * <p>
 * USAGE: To use this, You need to first register your custom NMS class in
 * {@link #registerEnchantmentHandle(Class)} which needs to extend either Bukkit
 * or NMS Enchantment class
 * and implement {@link NmsEnchant}
 * <p>
 * TIP: If you want to register for custom events, make sure that the event
 * actually happened when the
 * enchantment using {@link SimpleEnchantment#hasEnchant(ItemStack)} - DO NOT
 * USE BUKKIT'S CONTAINENCHANTMENT
 * METHOD because some Minecraft versions report it to false (or poor
 * implementations do - check that you have properly
...
public abstract class SimpleEnchantment implements Listener {
    ...
    protected SimpleEnchantment(@NonNull String name, int maxLevel) {
        ...
        Valid.checkBoolean(VALID_NAMESPACE.matcher(namespacedName).matches(),
                "Enchant name must only contain English alphabet names: " + name);
    ...
    private NmsEnchant assignHandle() {
        ...
        try {
            ...
            throw new FoException("Please add one public constructor taking SimpleEnchantment as one parameter to your "
                    + handleClass);
        }
    ...
    /**
     * Triggered automatically when the player clicks block/air with the given
     * enchant
     *
     * @param level
     * @param event
    ...
    /**
     * Triggered automatically when the player breaks block having hand item with
     * this enchantment
     *
     * @param level
     * @param event
     ...
     * <p>
     * We have to add item lore manually since Minecraft does not really support
     * custom
     * enchantments
     *
     * @param level
     ...
     * <p>
     * Use new NamespacedKey(SimplePlugin.getInstance(), this.name)) to convert on
     * MC 1.13+ to a {@link NamespacedKey}.
     *
     * @return
     */
    ...
    public static boolean hasEnchantment(ItemStack item, @NonNull SimpleEnchantment simpleEnchantment) {
        ...
        for (final Entry<Enchantment, Integer> entry : vanilla.entrySet()) {
            ...
            if (otherSimpleEnchantment != null
                    && otherSimpleEnchantment.getNamespacedName().equals(simpleEnchantment.getNamespacedName()))
                return true;
        }
    ...
    public static ItemStack addEnchantmentLores(ItemStack item) {
        ...
        if (!customEnchants.isEmpty()) {
            ...
            for (final String customEnchant : customEnchants) {
                ...
                final String enchantNoRoman = colorlessEnchant.substring(0, colorlessEnchant.lastIndexOf(" "));

                if (colorlessOriginals.isEmpty()) {
    ...
    public static final class Listener implements org.bukkit.event.Listener {
        ...
        public void onEntityDamage(EntityDamageByEntityEvent event) {
            ...
            if (damager instanceof LivingEntity)
                this.execute((LivingEntity) damager,
                        (enchant, level) -> enchant.onDamage(level, (LivingEntity) damager, event));
        }
        ...
        public void onShoot(ProjectileLaunchEvent event) {
            try {
                ...
                if (projectileSource instanceof LivingEntity) {
                    ...
                    EntityUtil.trackHit(event.getEntity(), hitEvent -> this.execute(shooter,
                            (enchant, level) -> enchant.onHit(level, shooter, hitEvent)));
                }
        ...
        private void execute(LivingEntity source, BiConsumer<SimpleEnchantment, Integer> executer) {
            try {
                final ItemStack hand = source instanceof Player ? ((Player) source).getItemInHand()
                        : source.getEquipment().getItemInHand();

                if (hand != null)
                    for (final Entry<SimpleEnchantment, Integer> e : SimpleEnchantment.findEnchantments(hand)
                            .entrySet())
