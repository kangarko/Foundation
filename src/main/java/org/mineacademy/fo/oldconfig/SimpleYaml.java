package org.mineacademy.fo.oldconfig;
 ...
 *
 * @deprecated not so frustrating free after all
 */
@Deprecated
final class SimpleYaml extends FileConfiguration {
	...
	public SimpleYaml() {
		...
		if (ReflectionUtil.isClassAvailable("org.yaml.snakeyaml.LoaderOptions")) {
			...
			try {
				...
				yaml = new Yaml(new YamlConstructor(), representer, dumperOptions, loaderOptions);

			} catch (final NoSuchMethodError ex) {
				yaml = new Yaml(new YamlConstructor(), representer, dumperOptions);
			}
	...
	public String saveToString(Map<String, Object> values) {
		...
		dumperOptions.setWidth(4096); // Foundation: Do not wrap long lines
