...
public class YamlConfig implements ConfigSerializable {
   ...
   * @return
   */
  protected static ConfigInstance findInstance(final String fileName) {
    for (final ConfigInstance instance : loadedFiles.keySet()) {
   ...
   *
   * <p>When you call methods with the "def" parameter, we enforce this flag and will NOT auto
   * update the config with the specified default value
   */
  ...
  /**
   * Internal flag whether to save the file after loading, set to true automatically when we edit it
   */
  private boolean save = false;

  /** Internal flag that can be toggled to disable working with default files. */
  @Setter private boolean usingDefaults = true;

  /**
   * Internal flag to indicate whether you are calling this from {@link #loadConfiguration(String,
   * String)}
   */
  ...
  /**
   * Loads up the localization file. These are stored in the localization/ folder and have the
   * syntax "messages_" + prefix + ".yml".
   *
   * <p>If the folder does not exists with the given file, we create it.
   *
   * <p>This method is intended to be called from the static config!
   *
   * @param localePrefix
   * @throws Exception
   */
   ...
   *
   * <p>The file must exist within out plugin at the same path since it will be copied to the plugin
   * folder if it does not exist, and it will be used to server as the default config to serve
   * updates from.
   *
   * @param file
   */
   ...
   *
   * <p>If the file does not exist, we create a new file. If you set "from" to null, no defaults
   * will be used.
   *
   * <p>Both paths must include file extension
   *
  ...
  /**
   * Replace variables in the file and write it again. For details see {@link
  ...
  /**
   * Replace variables in the destination file before it is copied. Variables include {plugin.name}
   * (lowercase), {file} and {file.lowercase} as well as custom variables from {@link
   * #replaceVariables(String)} method
   *
   * @param file
   */
  ...
  /**
   * Return the serialized map with all values you want to save to your config By default we return
   * an empty map
   *
  ...
  /**
   * Main configuration getter. Retrieve a value at a certain path, using type safety and default
   * configuration.
   *
   * <p>The type safety checks if the value is actually of the requested type, throwing errors if
   * not.
   *
   * <p>If {@link #getDefaults()} is set, and the value does not exist, we update the config file
   * automatically.
   *
   * @param <T>
   * @param path
  ...
  /**
   * Attempts to find the "public static T deserialize(SerializedMap) " method in the class type to
   * return the given path as the given class type,
   *
   * <p>if that fails then we try to look for "public static T getByName(String)" method in the
   * given type class,
   *
   * <p>if that fails than we attempt to deserialize it using {@link
   * SerializeUtil#deserialize(Class, Object)} method
   *
   * @param <T>
   * @param path
   * @param type
  ...
  /**
   * Basically the same as {@link #get(String, Class)} however you can pass your own deserialize
   * arguments here, see {@link SerializeUtil#deserialize(Class, Object, Object...)}
   *
   * @param <T>
   * @param path
   * @param type
   ...
   * @param type
   * @return
   * @see #getList(String, Class), except that this method never returns null, instead, if the key
   *     is not present, we return an empty set instead of null
   */
   ...
   *
   * <p>If the type is your own class make sure to put public static deserialize(SerializedMap)
   ...
   *
   * <p>If the type is your own class make sure to put public static deserialize(SerializedMap,
   * deserializedParameters) method into it that returns the class object from the map!
   *
   * @param <T>
   * @param path
   * @param type
  ...
  /**
   * Attempts to convert objects into strings, since SnakeYAML parser interprets "true" and "yes" as
   * boolean types
   *
   * @param list
   * @return
  ...
  public final <Key, Value> LinkedHashMap<Key, Value> getMap(
    ...
    for (final Map.Entry<String, Object> entry :
        getConfig().getConfigurationSection(path).getValues(false).entrySet()) {
  ...
  protected final LinkedHashMap<String, LinkedHashMap<String, Object>> getValuesAndKeys(
    ...
    if (getDefaults() != null && !getConfig().isSet(path)) {
      ...
      for (final String name : getDefaults().getConfigurationSection(path).getKeys(false)) {
        for (final String setting :
            getDefaults().getConfigurationSection(path + "." + name).getKeys(false)) {
    ...
    for (final String name : getConfig().getConfigurationSection(path).getKeys(false)) {
      // type, value (UNPARSED)
      final LinkedHashMap<String, Object> valuesRaw =
          getMap(path + "." + name, String.class, Object.class);
   ...
   *
   * <p>The file is not saved, however it is marked for save so it is saved at the end of a loading
   * cycle, if you call {@link #loadConfiguration(String)} or {@link #loadLocalization(String)}
   *
   * @param path
   * @param value
   */
   ...
   *
   * <p>Automatically saves the config at the end
   *
   * @param <O>
   * @param <N>
   * @param path
  ...
  protected final <T> T getOrSetDefault(final String path, final T defaultValue) {
    final Optional< Object> raw = Optional.ofNullable(getConfig().get(formPathPrefix(path)));

    final T result = (T) raw.orElseGet(() -> {
      save(path, defaultValue);
      return defaultValue;
    });

    Valid.checkNotNull(result, "Result mustn't be null");
    return result;
  }
  ...
  /**
   * Return whether the default config exist in your plugins jar and contains the given relative
   * path using {@link #getPathPrefix()} feature
   *
   * @param path
   * @return
   */
  ...
  /**
   * Return whether the default config exist in your plugins jar and contains the given absolute
   * path
   *
   * @param path
   * @return
   ...
   *
   * <p>We do not allow that, please call methods without the def parameter when using default
   * config as the default key will be fetched directly from the default config.
   *
   * @param path
   */
  ...
  /**
   * Checks if the file instance exists and is a valid file, checks if default exists, sets the save
   * flag to true and logs the update
   *
   * @param <T>
   * @param path
   * @param def
  ...
  protected String formPathPrefix(@NonNull final String path) {
    final String prefixed =
        pathPrefix != null ? pathPrefix + (!path.isEmpty() ? "." + path : "") : path;
  ...
  /**
   * A simple helper class for some language-specific values when creating localization for numbers.
   */
  ...
  public static final class LocationList implements Iterable<Location> {
    ...
    /**
     * Shortcut for adding/removing locations. Returns true if the given location not existed and it
     * was removed or returns false if it was found and removed.
     *
     * @param location
     * @return
     */
    ...
    /**
     * Return a validated location from the given location Pretty much the same but no yaw/pitch
     *
     * @param location
     * @return
     */
 ...
 *
 * <p>This represents the access to that file.
 */
...
class ConfigInstance {
  ...
  /**
   * Returns true if the given object is a ConfigInstance having the same file name, or a file with
   * the same name as this config instance
   *
   * @param obj
   * @return
   */
  ...
  public boolean equals(final Object obj) {
    return obj instanceof ConfigInstance
        ...
        : obj instanceof File
            ...
            : obj instanceof String && obj.equals(file.getName());
  }
}
